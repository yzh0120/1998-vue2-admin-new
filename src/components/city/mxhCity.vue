<template>
  <div>
    <el-col :span="4">
      <el-form-item :prop="province" :rules="{ required: required, message: '此项不能为空', trigger: 'change' }" label-width="0px">
        <el-select :disabled="disabled" v-model.trim="item[province]" placeholder="选择省"
          ...
          filterable>
          <el-option v-for="(key,index) in division" :value="index" :key="`${index}1`" :label="index">
          </el-option>
        </el-select>
      </el-form-item>
    </el-col>
    <el-col :span="4">
      <el-form-item :prop="city" :rules="{ required: required, message: '此项不能为空', trigger: 'change' }"  label-width="0px">
        <el-select :disabled="disabled" v-model.trim="item[city]" placeholder="选择市"
          v-on:change="() => { getAreas(item[province],item[city]); item[areas] = ''; }" :loading="loading" filterable>
          <el-option v-for="(key,index) in cityArr" :value="index" :key="`${index}2`" :label="index">
          </el-option>
        </el-select>
      </el-form-item>
    </el-col>
    <el-col :span="4">
      <el-form-item :prop="areas" :rules="{required:required,message:'此项不能为空',trigger:'change'}"  label-width="0px">
        <el-select v-model.trim="item[areas]" placeholder="选择区" @change="areaChange" :loading="loading">
          ...
          <el-option v-for="(key,index) in areaArr" :value="key" :key="`${index}3`" :label="key"></el-option>
        </el-select>
      </el-form-item>
    </el-col>
    <el-col :span="12">
      <el-form-item :prop="adress" :rules="{ required: addressRequired, message: '此项不能为空', trigger: 'blur' }"  label-width="0px"
        v-if="adress">
...
export default {
  ...
  methods: {
    ...
    getTree() {
      // this.loading = true;
      // administrativedivision.getTree({ parentId: "" }).then((res) => {
      //   this.loading = false;
    ...
    async getCitys(province) {
      ...
      // this.cityArr = [];
      if (province) { 
        this.cityArr = this.division[province]
      }
    },
    async getAreas(province,city) {
      ...
      if (!!province && !!city) {
                if (this.division[province]) {
                  this.areaArr = this.division[province][city];
                } else {
                  this.areaArr =  []
                }
            } else {
